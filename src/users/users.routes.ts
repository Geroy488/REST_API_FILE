 ...
 userRouter.get("/users", async (req: Request, res: Response) => {
  ...
  userRouter.get("/user/:id", async (req : Request, res : Response) => {
    try{
        ...
        if(!user) {
            return res.status(StatusCodes.NOT_FOUND).json({error : `User not founf!`})
        }
  ...
  userRouter.post("/login", async (req : Request, res : Response) => {
    try {
        ...
        if(!user) {
            return res.status(StatusCodes.NOT_FOUND).json({error : "No user exist with the email provides.."})
        }
